 ...
 * Improved by unnamed777 <andrey@neyman.tel>
 */
...
<html>
<head>
<title>1C Diagnostics Tool</title>
<script>var bxSession = {mess: {}};</script>
...
<?
    ...
    ob_start();
    ShowFileSelect('cat_file','Файл каталога в '.$UPLOAD_DIR.'/1c_catalog/:',$UPLOAD_DIR.'/1c_catalog/','xml',5,'ConfirmImport(\'catalog\')');
    print '<br/><button onclick="confirm(\'Запустить деактивацию?\') ? app.importDeactivate() : false;">Деактивация</button> <input type="text" size="18" name="importDeactivationTime" title="Будут деактивированы разделы и элементы, изменённые раньше этой даты" value=""/><br/><br/>';
...
function CatalogImport()
{
    ...
    load.innerHTML='идёт загрузка...<img align="center" src="http://gifanimation.ru/images/mehanizmy_new/Gears-03.gif" width="30"/>';
    query_1c(url);
}
...
app.importInit = function (originalUrl) {
    "use strict";
    ...
    url = app.scriptName + '?type=catalog&mode=checkauth';
    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.status == 0) {
            app.addError('Инициализация - пустой ответ сервера');
            return;
        }

        if (xhr.responseText.substr(0, 7) == 'success') {
            var params = xhr.responseText.split('\n');

            for (var i = 0; i < params.length; i++) {
                switch (true) {
                    case params[i].indexOf('sessid=') !== -1:
                        app.sessidParam = params[i];
                        break;

                    case params[i].indexOf('timestamp=') !== -1:
                        app.timestampParam = params[i];
                        app.importStartTimestamp = params[i].split('=')[1];
                        break;
                }
            }

            var date = new Date();
            date.setTime(app.importStartTimestamp * 1000);
            var lastImportTime = date.toISOString().substr(0, 19);
            document.querySelector('[name=importDeactivationTime]').value = lastImportTime;

            // Initialization is completed, continue old procedures
            query_1c(originalUrl);
        } else {
            app.addError('Инициализация - ошибка:\n' + xhr.responseText);
        }
    };
...
app.importDeactivate = function () {
    "use strict";
    var lastImportTime = document.querySelector('[name=importDeactivationTime]').value,
        date,
        timestamp,
        xhr;

    if (lastImportTime.length != 19) {
        alert('Укажите дату в формате YYYY-MM-DDTHH:mm:ss');
        return;
    }

    date = new Date(lastImportTime + '.000Z');
    ...
    log.innerHTML = log.innerHTML + 'Деактивация разделов и элементов (&lt; ' + lastImportTime + ')<br/>';
    load.innerHTML='идёт загрузка...<img align="center" src="http://gifanimation.ru/images/mehanizmy_new/Gears-03.gif" width="30"/>';
    load.style.display = "block";

    xhr = createHttpRequest();
    url = app.scriptName + '?type=catalog&mode=deactivate&timestamp=' + timestamp.toString() + '&' + app.sessidParam;
    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.status == 0) {
            app.addError('Пустой ответ сервера');
            load.style.display = "none";
            return;
        }
        ...
        if (xhr.responseText.substr(0, 7) == 'success') {
            ...
            log.innerHTML = log.innerHTML + 'Деактивация разделов и элементов завершена.<br/>';
            load.style.display = "none";
        } else {
            app.addError('Ошибка:\n' + xhr.responseText);
            load.style.display = "none";
...
app.addError = function (error) {
    "use strict";
    // @todo Get rid of using global i
    var error_text = '<em>Ошибка в процессе выгрузки</em><div style="width: 270px; font-size: 11px; border: 1px solid black; background-color: #ADC3D5; padding: 5px;">' + error + '</div>';
    log.innerHTML = log.innerHTML + 'Шаг ' + i + ': ' + error_text;
};

//функция осуществляет импорт из файла
function query_1c(url) {
    // There is no session id, obtain it
    if (!app.sessidParam) {
        app.importInit(url);
        return;
    }
...
bxtabs.TabCreate('Файлы');
bxtabs.TabCreate('Импорт файлов 1C', true);
